...
byte* CN64MidiToolDlg::Decompress(unsigned char* Buffer, unsigned long size, int& fileSize)
{
...
bool CN64MidiToolDlg::DecompressToFile(unsigned char* Buffer, unsigned long size, CString outputFile)
{
	...
	byte* outputDecompressed = Decompress(Buffer, size, fileSize);
...
void CN64MidiToolDlg::ExportToMidi(unsigned char* gamebuffer, unsigned long address, unsigned long size, CString fileName, CString gameType, int& numberInstruments, unsigned long division)
{
	if (gameType.Find("BanjoTooie") == 0)
	{
		if (compressed)
		{
			...
			byte* outputBuffer = Decompress(&gamebuffer[address], size, decompressedSize);
	...
	else
	{
		...
		{
			int decompressedSize = 0;
			byte* outputBuffer = Decompress(&gamebuffer[address], (size + 20000), decompressedSize);
...
void CN64MidiToolDlg::OnBnClickedButtonplaymidi()
{
	...
	CString sParameter;
		ShellExecute( GetSafeHwnd(), "open", "explorer.exe", (mainFolder + "tempAS123123as.mid"),
mainFolder, SW_SHOWNORMAL );
